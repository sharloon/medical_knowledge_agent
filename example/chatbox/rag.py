...
def ask_stream(question: str, query_engine) -> Generator[str, None, None]:
    """
    向答疑机器人提问，以流式方式逐步返回回答。
    
    参数:
        question (str): 用户提出的问题
        query_engine: 查询引擎对象（支持流式响应）

    返回:
        Generator[str, None, None]: 逐步返回生成的回答内容
    """
    if not question:
        yield "错误：问题不能为空"
        return
    ...
    try:
        # 假设 query_engine.query 返回一个支持流式响应的对象
        streaming_response = query_engine.query(question)

        # 假设 response_gen 是一个生成器，逐步生成回答内容
